...
export default function Home() {
  ...
  return (
    <div className="container laptopl:mx-3 mx-auto mb-10">
      <Header
        handleWorkScroll={handleWorkScroll}
        handleAboutScroll={handleAboutScroll}
      />
      <div className="laptop:mt-20 mob:mt-10">
        <h1 className="mt-5 text-6xl mob:text-3xl laptop:text-6xl mob:p-2 text-bold w-4/5 mob:w-full laptop:w-4/5">
          {data.headerTaglineOne} <br />
          {data.name} <br />
          {data.headerTaglineTwo}
        </h1>
        <Socials className="mt-5 mob:mt-2 laptop:mt-5" socials={data.socials} />
      </div>
      <div
